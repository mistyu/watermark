...
public class AmapFlutterSearchPlugin implements FlutterPlugin, MethodCallHandler,
  ...
  private void searchAround(Map searchParams,Result result) throws AMapException {
    if (null != searchParams) {
      ...
      // 设置默认值避免空指针
      int radius = searchParams.get("radius") == null ? 1000 : ((Integer)searchParams.get("radius")).intValue();
      int pageSize = searchParams.get("pageSize") == null ? 20 : ((Integer)searchParams.get("pageSize")).intValue();
      int page = searchParams.get("page") == null ? 1 : ((Integer)searchParams.get("page")).intValue();
  ...
  public void onPoiSearched(PoiResult poiResult, int i) {
     ...
     if (poiResult == null || poiResult.getPois() == null) {
      resultCallback.success(pois);  // 返回空列表
      return;
    }
